-- lua/block_comment/parser.lua
-- Treesitter-based block detection for block_comment plugin.
...
local function label_while_loop(node, src)
  local cond = field_text(node, "condition", src)
  if cond ~= "" then return "while " .. trunc(cond) end
  return "while"
end

local function label_if(node, src)
  local cond = field_text(node, "condition", src)
  if cond ~= "" then return "if " .. trunc(cond) end
  return "if"
end
...
local function label_function_def(node, src)
  local name = field_text(node, "name", src)
  if name ~= "" then return "function " .. name end
...
local lang_configs = {
  ...
  cpp = {
    ...
    { types = { "for_range_loop" },          fn = function(node, src)
    ...
    { types = { "do_statement" },            fn = function() return "do" end },
    { types = { "if_statement" },            fn = label_if         },
    { types = { "else_clause" },             fn = function() return "else" end },
    ...
    { types = { "switch_statement" },        fn = function(n, s)
  ...
  -- ── Rust ───────────────────────────────────────────────────────────────────
  rust = {
    { types = { "for_expression" }, fn = function(node, src)
    ...
    { types = { "loop_expression" },         fn = function() return "loop" end },
    { types = { "if_expression" },           fn = label_if         },
    { types = { "else_clause" },             fn = function() return "else" end },
    { types = { "function_item" },           fn = label_function   },
    { types = { "impl_item" },               fn = function(node, src)
    ...
    { types = { "enum_item" },               fn = function(node, src)
        local name = field_text(node, "name", src)
        return "enum " .. name
      end },
    { types = { "mod_item" },                fn = function(node, src)
        local name = field_text(node, "name", src)
        return "mod " .. name
      end },
    { types = { "match_expression" },        fn = function(node, src)
        local val = field_text(node, "value", src)
        return "match " .. trunc(val)
      end },
    { types = { "closure_expression" },      fn = function() return "closure" end },
  },

  -- ── Zig ────────────────────────────────────────────────────────────────────
  zig = {
    { types = { "for_statement", "for_expression" }, fn = function(node, src)
        -- Zig: for (slice) |elem|
        local text = vim.treesitter.get_node_text(node, src)
        local cap  = text:match("for%s*(%b())")
        return cap and ("for " .. trunc(cap)) or "for"
      end },
    { types = { "while_statement", "while_expression" }, fn = function(node, src)
        local text = vim.treesitter.get_node_text(node, src)
        local cap  = text:match("while%s*(%b())")
        return cap and ("while " .. trunc(cap)) or "while"
      end },
    { types = { "if_statement", "if_expression" }, fn = function(node, src)
        local text = vim.treesitter.get_node_text(node, src)
        local cap  = text:match("if%s*(%b())")
        return cap and ("if " .. trunc(cap)) or "if"
      end },
    { types = { "fn_decl", "function_declaration" }, fn = label_function },
    { types = { "struct_decl", "container_decl" },  fn = function(node, src)
  ...
  -- ── Lua ────────────────────────────────────────────────────────────────────
  lua = {
    { types = { "for_statement" }, fn = function(node, src)
        local text = vim.treesitter.get_node_text(node, src)
  ...
  python = {
    ...
    { types = { "with_statement" }, fn = function(node, src)
        local text = vim.treesitter.get_node_text(node, src)
        local cap  = text:match("^with%s+(.-)%s*:")
        return cap and ("with " .. trunc(cap)) or "with"
      end },
    { types = { "try_statement" },  fn = function() return "try" end },
    { types = { "except_clause" },  fn = function(node, src)
        local text = vim.treesitter.get_node_text(node, src)
  ...
  nim = {
    ...
    { types = { "macro_declaration" },    fn = function(node, src)
        local name = field_text(node, "name", src)
        if name ~= "" then return "macro " .. name end
        return "macro"
      end },
    { types = { "template_declaration" }, fn = function(node, src)
        local name = field_text(node, "name", src)
        if name ~= "" then return "template " .. name end
        return "template"
      end },
    { types = { "converter_declaration" }, fn = function(node, src)
        local name = field_text(node, "name", src)
        if name ~= "" then return "converter " .. name end
        return "converter"
      end },
    { types = { "concept_declaration" },  fn = function() return "concept" end },

    -- Type declarations
    -- type_declaration has type_symbol_declaration (name) as a child
    { types = { "type_declaration" }, fn = function(node, src)
        for child in node:iter_children() do
    ...
    { types = { "object_declaration" }, fn = function(node, src)
        local p = node:parent()
    ...
    { types = { "enum_declaration" }, fn = function(node, src)
        local p = node:parent()
        if p and p:type() == "type_declaration" then
          for child in p:iter_children() do
            if child:type() == "type_symbol_declaration" then
    ...
    { types = { "type_section" }, fn = function() return "type" end },

    -- Loops
    { types = { "for" }, fn = function(node, src)
    ...
    { types = { "while" }, fn = function(node, src)
        local cond = field_text(node, "condition", src)
        if cond ~= "" then return "while " .. trunc(cond) end
        return "while"
      end },
    ...
    { types = { "case" }, fn = function(node, src)
        local val = field_text(node, "value", src)
        if val ~= "" then return "case " .. trunc(val) end
        return "case"
      end },
    { types = { "elif_branch" }, fn = function(node, src)
        local cond = field_text(node, "condition", src)
        if cond ~= "" then return "elif " .. trunc(cond) end
        return "elif"
      end },
    { types = { "else_branch" }, fn = function() return "else" end },
    { types = { "of_branch" }, fn = function(node, src)
        local vals = field_text(node, "values", src)
        if vals ~= "" then return "of " .. trunc(vals) end
        return "of"
      end },

    -- Error handling
    { types = { "try" }, fn = function() return "try" end },
    { types = { "except_branch" }, fn = function(node, src)
        local vals = field_text(node, "values", src)
        if vals ~= "" then return "except " .. trunc(vals) end
        return "except"
      end },
    { types = { "finally_branch" }, fn = function() return "finally" end },

    -- Named block
    { types = { "block" }, fn = function(node, src)
        local label = field_text(node, "label", src)
        if label ~= "" then return "block " .. label end
        return "block"
      end },

    -- Other
    { types = { "do_block" },         fn = function() return "do" end },
    { types = { "defer" },            fn = function() return "defer" end },
    { types = { "const_section" },    fn = function() return "const" end },
    { types = { "let_section" },      fn = function() return "let" end },
    { types = { "var_section" },      fn = function() return "var" end },
    { types = { "static_statement" }, fn = function() return "static" end },
  },
}
...
function M.get_label(lnum, bufnr)
  ...
  if not ok or not parser_obj then
    vim.notify("[block_comment] No treesitter parser for " .. ft, vim.log.levels.WARN)
    return nil
  end
