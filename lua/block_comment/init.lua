-- lua/block_comment/init.lua
-- Main entry point for the block_comment Neovim plugin.
--
-- Commands
...
local M    = {}
local parser = require("block_comment.parser")
...
local function is_closing_line(line, ft)
  ...
  for _, tok in ipairs(tokens) do
    if bare == tok then return true end
  end
  return false
end
...
function M.add_comment()
  ...
  local tpl   = config.comment_style[ft]
  if not tpl then
    vim.notify("[block_comment] Filetype '" .. ft .. "' not configured", vim.log.levels.WARN)
    return
  end
  ...
  if not is_closing_line(line, ft) then
    vim.notify("[block_comment] Cursor is not on a closing line", vim.log.levels.WARN)
    return
  end

  -- Guard: already has our comment → skip
  if has_our_comment(line, ft) then
    vim.notify("[block_comment] Comment already present", vim.log.levels.INFO)
    return
  end

  local label = parser.get_label(lnum, bufnr)
  if not label then
    vim.notify("[block_comment] Could not identify block type", vim.log.levels.INFO)
    return
  end
...
function M.remove_comment()
  ...
  if not has_our_comment(line, ft) then
    vim.notify("[block_comment] No block_comment found on this line", vim.log.levels.INFO)
    return
  end
...
function M.add_all()
  ...
  vim.fn.setpos(".", saved)
  vim.notify(("[block_comment] Added %d comment(s)"):format(count), vim.log.levels.INFO)
end

--- Remove all block_comment comments in the buffer.
function M.remove_all()
  ...
  vim.fn.setpos(".", saved)
  vim.notify(("[block_comment] Removed %d comment(s)"):format(count), vim.log.levels.INFO)
end
...
function M.setup(opts)
  ...
  vim.api.nvim_create_user_command("BlockCommentRemove", function()
    M.remove_comment()
  end, { desc = "Remove block_comment comment from current closing line" })
  ...
  vim.api.nvim_create_user_command("BlockCommentRemoveAll", function()
    M.remove_all()
  end, { desc = "Remove all block_comment comments in buffer" })
